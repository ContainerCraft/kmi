...
#===============================================================================
# Fedora 42 VM Deployment Script
#===============================================================================
# This script automates the deployment of Fedora 42 VirtualMachine with:
...
USERDATA_PATH="${SCRIPT_DIR}/fedora-42-userdata.yaml"
VM_MANIFEST_PATH="${SCRIPT_DIR}/fedora-42-vdi-xrdp-gnome-br0-containerdisk.yaml"
NAMESPACE="default"
SKIP_SECRETS=false
MONITOR=false

# Secret names
SSH_SECRET_NAME="kargo-sshpubkey-kc2user"
USERDATA_SECRET_NAME="fedora-42-userdata"
...
if [[ "$MONITOR" == true ]]; then
    ...
    if [[ -n "$VM_IP" ]]; then
        ...
        echo "═══════════════════════════════════════════════════════════"
        echo "  VM IP:         $VM_IP"
        echo "  SSH:           ssh kc2user@$VM_IP"
        echo "  RDP:           xfreerdp /v:$VM_IP:3389 /u:kc2user /p:kc2user /cert:ignore"
        echo "  Console:       virtctl console $VM_NAME -n $NAMESPACE"
        echo ""
        echo "  User:          kc2user"
        echo "  Password:      kc2user"
        echo "═══════════════════════════════════════════════════════════"
    else
